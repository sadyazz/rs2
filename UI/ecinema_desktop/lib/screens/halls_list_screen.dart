...
class _HallsListScreenState extends State<HallsListScreen> {
  late HallProvider provider;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    provider = context.read<HallProvider>();
  }
  ...
  Future<void> _searchHalls() async {
    try {
      setState(() {
        isLoading = true;
      });
  ...
  void _deleteHall(Hall hall) async {
    final l10n = AppLocalizations.of(context)!;
    final hallName = hall.name ?? l10n.unnamedHall;
    
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(l10n.deleteHall),
        content: Text(l10n.confirmDeleteHall(hallName)),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      try {
        await provider.softDelete(hall.id!);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.hallDeletedSuccessfully),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.failedToDeleteHall),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  void _restoreHall(Hall hall) async {
    final l10n = AppLocalizations.of(context)!;
    final hallName = hall.name ?? l10n.unnamedHall;
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(l10n.confirmRestoreHall(hallName)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: Text(l10n.close),
    ...
    if (confirmed == true) {
      try {
        await provider.restore(hall.id!);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.hallRestoredSuccessfully),
            backgroundColor: Colors.green,
          ),
        );
        _resetPagination();
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.failedToRestoreHall),
            backgroundColor: Colors.red,
          ),
        );
      }
                    ...
                    Row(
                      ...
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                          decoration: BoxDecoration(
                            color: hall.isActive == true 
                                ? Colors.green[100] 
                                : Colors.red[100],
                            borderRadius: BorderRadius.circular(8),
                          ),
                          child: Text(
                            hall.isActive == true ? l10n.active : l10n.inactive,
                            style: TextStyle(
                              color: hall.isActive == true 
                                  ? Colors.green[700] 
                                  : Colors.red[700],
